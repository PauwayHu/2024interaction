...
PFont font0, font, font2; //3種字形: 英文 中文 注音文(外面宣告,兩個函式都看的到)
void setup(){
   size(500, 500); //大視窗
   font0 = createFont("Algerian", 50); // Arial 或 Times New Roman
   font = createFont("標楷體", 50);
   font2 = createFont("elffont-rock.otf", 50); //字形檔,也要拉進來
   // println(PFont.list()); //印出現在能用的系統的所有字形
}
void draw(){
   background(0); //黑色背景
   textSize(50);
   //PFont font0 = creatFont("Ariel"50); 不能寫在裡面,見幾萬次字形電腦會亂掉
   textFont(font0);
   text("Hello",50, 50);
